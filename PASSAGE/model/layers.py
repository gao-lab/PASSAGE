...
from torch.nn import Parameter, Dropout, Linear, LayerNorm
...
class AttentionModule(torch.nn.Module):
    ...
    def setup_weights(self):
        ...
        Defining weights.
        """
        self.weight_matrix = torch.nn.Parameter(torch.Tensor(self.filter_size,
                                                             self.filter_size))
    ...
    def forward(self, embedding):
        """
        Making a forward propagation pass to crea te a graph level representation.
        :param embedding: Result of the GCN.
        :return representation: A graph level representation vector.
        """
        global_context = torch.mean(torch.matmul(embedding, self.weight_matrix), dim=0)
        sigmoid_scores = torch.sigmoid(torch.mm(embedding, global_context.view(-1, 1)))
        sigmoid_scores = F.normalize(sigmoid_scores, dim=0)
        representation = torch.mm(torch.t(embedding), sigmoid_scores)
        representation = representation/(torch.norm(representation, p=2)+1e-8)
        return sigmoid_scores, representation